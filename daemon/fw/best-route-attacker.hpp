 ...
 */
class BestRouteStrategyAttacker : public Strategy
                         , public ProcessNackTraits<BestRouteStrategyAttacker>
{
public:
  explicit
  BestRouteStrategyAttacker(Forwarder& forwarder, const Name& name = getStrategyName());

  static const Name&
  getStrategyName();
  
  struct nameDetails{
  	nameDetails() : count(0), payload(0)  {}
        nameDetails(int newcount, int newpayload)
         : count(newcount), payload(newpayload) {} 
        int count;
        int payload;
  };
  
//  extern static std::map<std::string,nameDetails> nameMap;
...
PUBLIC_WITH_TESTS_ELSE_PRIVATE:
  ...
  friend ProcessNackTraits<BestRouteStrategyAttacker>;

private:
  //struct BestRouteStrategyAttacker::nameDetails namedetails;
  std::unordered_map<Name, BestRouteStrategyAttacker::nameDetails> nameMap;
  double checktime =10;
...
#endif // NFD_DAEMON_FW_BEST_ROUTE_STRATEG_Attacker_HPP
